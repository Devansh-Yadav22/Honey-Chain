...
export const HiveDetailPage: React.FC<HiveDetailPageProps> = ({ hiveId, onBack }) => {
  ...
  return (
    <div className="space-y-6">
      ...
      >
        <ArrowLeft className="w-4 h-4 mr-1.5" /> Back to Hive List
      </button>
      ...
      <div className="panel-card p-6 rounded-2xl flex flex-col md:flex-row items-start md:items-center justify-between gap-4 shadow-sm border border-stone-200/80">
        <div>
          <div className="flex items-center space-x-3 flex-wrap gap-y-1">
            <span className="text-2xl font-bold font-mono text-stone-900" style={{ fontFamily: "'Outfit', sans-serif" }}>{hive.id}</span>
            <StatusBadge status={hive.status} />
          </div>
          <p className="text-xs text-stone-600 mt-1.5 font-medium">{hive.location.address || 'Apiary Location'}</p>
        </div>
        ...
        <div className="flex items-center space-x-6 border-t md:border-t-0 md:border-l border-stone-200 pt-3 md:pt-0 md:pl-6 text-xs">
          <div>
            ...
            <p className={`text-2xl font-bold ${health?.health === 'CRITICAL' ? 'text-rose-800' : health?.health === 'WARNING' ? 'text-amber-800' : 'text-[#3D5A3A]'}`} style={{ fontFamily: "'Outfit', sans-serif" }}>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="panel-card p-4 rounded-2xl space-y-1 border border-stone-200/80">
          <div className="flex items-center text-xs font-semibold text-amber-700">
            <Thermometer className="w-4 h-4 mr-1.5" /> Temperature
          </div>
          <p className="text-2xl font-bold text-stone-900" style={{ fontFamily: "'Outfit', sans-serif" }}>{latest.temperature}°C</p>
          <p className="text-[11px] text-stone-500 font-medium">Optimal range: 33°C - 36°C</p>
        </div>
        ...
        <div className="panel-card p-4 rounded-2xl space-y-1 border border-stone-200/80">
          ...
          <p className="text-2xl font-bold text-stone-900" style={{ fontFamily: "'Outfit', sans-serif" }}>{latest.humidity}%</p>
          <p className="text-[11px] text-stone-500 font-medium">Optimal range: 55% - 65%</p>
        </div>

        <div className="panel-card p-4 rounded-2xl space-y-1 border border-stone-200/80">
          <div className="flex items-center text-xs font-semibold text-[#3D5A3A]">
            <Scale className="w-4 h-4 mr-1.5 text-[#3D5A3A]" /> Hive Weight
          </div>
          <p className="text-2xl font-bold text-stone-900" style={{ fontFamily: "'Outfit', sans-serif" }}>{latest.weight} kg</p>
          <p className="text-[11px] text-stone-500 font-medium">Estimated Honey Yield: 18.4 kg</p>
        </div>
        ...
        <div className="panel-card p-4 rounded-2xl space-y-1 border border-stone-200/80">
          ...
          <p className="text-2xl font-bold text-stone-900" style={{ fontFamily: "'Outfit', sans-serif" }}>{Math.round(latest.activity * 100)}%</p>
          <p className="text-[11px] text-stone-500 font-medium">Foraging Flight Index</p>
        </div>
      </div>
      ...
      {anomalies?.anomaly && (
        <div className="p-5 rounded-2xl border border-rose-200 bg-rose-50/90 text-rose-900 space-y-2 shadow-sm">
          <div className="flex items-center space-x-2 text-rose-800 font-bold text-sm" style={{ fontFamily: "'Outfit', sans-serif" }}>
